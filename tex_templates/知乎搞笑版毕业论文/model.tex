...
\begin{figure}[H]% 插入一张图片，H表示浮动环境下的here
	...
	\begin{minipage}{0.83\textwidth}% 小页面尺寸，可自行调节
		...
		\textwidth]{tamiyo}% 图片名称（图片需与tex文件在同一文件夹）
		\caption{\fontsize{10pt}{15pt}\selectfont 俺美美滴老婆}% 图例
	\end{minipage}
\end{figure}
...
\begin{figure}[H]% 插入两张图片并且并排
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=0.83\textwidth]{chuanda1}
		\caption{\fontsize{10pt}{15pt}\selectfont 穿搭}
	\end{minipage}
	\hspace{0cm}% 图片间距
	\hfill% 撑满整行
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=0.83\textwidth]{chuanda2}
		\caption{\fontsize{10pt}{15pt}\selectfont 也是穿搭}
	\end{minipage}
\end{figure}
...
\begin{table}[H]% 插入表格
	...
	\end{tabular}
\caption{\fontsize{10pt}{15pt}\selectfont 表(不应该是表上图下吗?)}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 三线表 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{table}[H]% 插入表格
	\centering
